...
describe('2.1.3.1: When rejected, a promise: must not transition to any other state.', () => {
  ...
  it("trying to reject then immediately fulfill", (done) => {
    const d = deferred()
    let onRejectedCalled = false
  ...
  it("trying to reject then fulfill, delayed", (done) => {
    const d = deferred()
    let onRejectedCalled = false
  ...
  it("trying to reject immediately then fulfill delayed", (done) => {
    const d = deferred()
    let onRejectedCalled = false
